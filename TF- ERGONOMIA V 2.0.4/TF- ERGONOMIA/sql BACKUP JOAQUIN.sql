...
use ProyectoFinal
go
CREATE OR ALTER PROCEDURE SP_Empleados_Conpuestoscargado
@DNI nvarchar(8)
AS
BEGIN
...
AS
BEGIN
SELECT Provincia, COUNT(Provincia) AS CantidadEmpresas FROM Empresas GROUP BY Provincia ORDER BY CantidadEmpresas DESC;
END
GO
...
AS
BEGIN
SELECT E.Nombre AS NombreEmpresa, COUNT(*) AS CantidadEmpleados
    FROM Empresas E JOIN Empleados EM ON E.IdEmpresa = EM.IdEmpresa
    WHERE E.CUIT = @CUIT AND EM.FechaIngreso BETWEEN @FechaIngreso AND @FechaIngreso2 AND EM.Estado = 'A'
	GROUP BY E.Nombre  ORDER BY CantidadEmpleados DESC;
END
GO
